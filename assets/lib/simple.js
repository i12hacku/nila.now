...
export function makeWaSocket(connectionOptions, options = {}) {
    ...
    let sock = Object.defineProperties(conn, {
        ...
        my_id: {
            get() {
                return this.decodeJid(this.user.id)
            }
        },
        logger: {
            get() {
...
END:VCARD
        ...
        /* load: {
            value(jid, text = 'sucess', quoted, options) {
                return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, {
                    ...options,
                    text
                }, {
...
END:VCARD`.trim()
        ...
        fakeReply: {
            ...
            value(jid, text = '', fakeJid = this.user.jid, fakeText = '', fakeGroupJid, options) {
                return conn.reply(jid, text, { key: { fromMe: areJidsSameUser(fakeJid, conn.user.id), id: '0@s.whatsapp.net', participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message: { conversation: fakeText }, ...options })
            }
        },
        fakeReply2: {
            /**
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        quoted: {
            get() {
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q)), {
                    ...
                    id: {
                        get() {
                        // console.log(contextInfo.quotedMessage.extendedTextMessage.contextInfo)
        ...
        load: {
            async value(text = 'sucses') {
                ...
                let { key } = this.conn?.sendMessage(this.chat, { text: 'test' })
    
                for (let i = 0; i < l.length; i++) {
                    this.conn?.sendMessage(this.chat, { text: l[i], edit: key });
                }
            }
        },
        copy: {
